...
#import "LJobJSONRequestSerializer.h"
#import "LJobQueryParamCollection.h"
#import "LJobDefaultConfiguration.h"

NSString *const LJobResponseObjectErrorKey = @"LJobResponseObject";

static NSString * const kLJobContentDispositionKey = @"Content-Disposition";

static NSDictionary * LJob__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * LJob__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = LJob__headerFieldsForResponse(response);
    if(!headers[kLJobContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kLJobContentDispositionKey];
...
@interface LJobApiClient ()

@property (nonatomic, strong, readwrite) id<LJobConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation LJobApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static LJobApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[LJobDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[LJobDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<LJobConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[LJobResponseDeserializer alloc] init];
        _sanitizer = [[LJobSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        LJobJSONRequestSerializer * swgjsonRequestSerializer = [LJobJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kLJobApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        LJobDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[LJobResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = LJob__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<LJobSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = LJobPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = LJobPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, LJobPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[LJobQueryParamCollection class]]){
            LJobQueryParamCollection * coll = (LJobQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = LJobPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = LJobPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = LJobPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<LJobConfiguration> config = self.configuration;
