...
NSString * const kLJobApplicationJSONType = @"application/json";

NSString * LJobPercentEscapedStringFromString(NSString *string) {
    static NSString * const kLJobCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kLJobCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kLJobCharactersGeneralDelimitersToEncode stringByAppendingString:kLJobCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[LJobQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[LJobObject class]]) {
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kLJobApplicationJSONType;
        }
