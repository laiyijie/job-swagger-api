...
#import "LJobBasicAuthTokenProvider.h"
#import "LJobLogger.h"

@interface LJobDefaultConfiguration ()
...
@implementation LJobDefaultConfiguration

#pragma mark - Singleton Methods

+ (instancetype) sharedConfig {
    static LJobDefaultConfiguration *shardConfig = nil;
...
- (instancetype) init {
    self = [super init];
    if (self) {
        _host = @"http://58.87.75.73:8888/job/api/v1/";
        _username = @"";
        _password = @"";
        _accessToken= @"";
        _verifySSL = YES;
        ...
        _logger = [LJobLogger sharedLogger];
    }
    return self;
}
...
- (NSString *) getBasicAuthToken {
    ...
    NSString *basicAuthToken = [LJobBasicAuthTokenProvider createBasicAuthTokenWithUsername:self.username password:self.password];
